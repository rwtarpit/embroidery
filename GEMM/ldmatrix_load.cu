...
namespace wt {
    ...
    __device__ void loadFromGmem(int N, int K, const float *A, const float *B,
        ...
        for (uint offset = 0; offset + rowStrideA <= BM; offset += rowStrideA) {
            cuda::memcpy_async(
                &As[((innerRowA + offset)*BK + innerColA * 4)],
    ...
    for (uint tile = 0; tile < NUM_TILES; ++tile) {
        ...
        for (int warp_tile_A = 0; warp_tile_A < TILES_PER_WARP_M; ++warp_tile_A) {
            for (int inner_tile = 0; inner_tile < BK / TILE_SIZE_K; ++inner_tile) {
                ...
                uint32_t As_ptr_ = __cvta_generic_to_shared(&As[cur_stage][(a_row)* BK + a_col]);
                ...
                for (int warp_tile_B = 0; warp_tile_B < TILES_PER_WARP_N; ++warp_tile_B) {
                    ...
                    int b_row = b_row_base + (lane_id % 8);
                    int b_col = b_col_base + ((lane_id / 8) % 2) * 4;
                    // for threads 16-31, mirror threads 0-15
                    
                    uint32_t Bs_ptr_ = __cvta_generic_to_shared(&Bs[cur_stage][b_row * BN + b_col]);
                    asm volatile(
